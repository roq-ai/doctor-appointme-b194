...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['admin-user', 'guest-user', 'doctor-user'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<PatientInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/patients` : null),
    () => getPatients({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Patient',
      path: isTenantUser ? `/patients/view/${data?.[0]?.id}` : '/patients',
      entity: 'patient',
    ...
    { name: 'Doctor', path: '/doctors', entity: 'doctor', service: AccessServiceEnum.PROJECT, icon: FiBox },
